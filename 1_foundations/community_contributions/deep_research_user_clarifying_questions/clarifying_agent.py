...
class ClarifyingQuestions(BaseModel):
    questions: list[str] = Field(
        description=f"A list of {HOW_MANY_CLARIFYING_QUESTIONS} clarifying questions to better understand the user's research query."
    )


class EnhancedQuery(BaseModel):
    original_query: str = Field(description="The original user query")
    clarifying_context: str = Field(
        description="A summary of the clarifying questions and user responses"
    )
    enhanced_query: str = Field(
        description="The enhanced search query incorporating user clarifications"
    )
