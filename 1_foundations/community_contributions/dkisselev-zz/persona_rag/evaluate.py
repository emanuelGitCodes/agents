...
class AnswerEval(BaseModel):
    ...
    accuracy: float = Field(
        description="How factually correct is the answer? 1 (wrong) to 5 (perfect)"
    )
    completeness: float = Field(
        description="How complete is the answer? 1 (missing key info) to 5 (comprehensive)"
    )
    relevance: float = Field(
        description="How relevant is the answer? 1 (off-topic) to 5 (directly addresses question)"
    )
...
def evaluate_chunks(
    chunks, tests, chunk_size, embeddings_model="thenlper/gte-small", k=9
):
...
def run_hyperparameter_tuning(chunk_sizes, k_values, sample_size=20):
    ...
    for k in k_values:
        ...
        mrr = evaluate_chunks(
            optimal_chunks, sampled_tests, int(best_chunk["chunk_size"]), k=k
        )
        elapsed = time.time() - start

        k_results.append({"k": k, "mrr": mrr, "time_seconds": elapsed})
        print(f"     MRR: {mrr:.4f}, Time: {elapsed:.1f}s")
...
def plot_chunk_results(chunk_df, best_chunk):
    ...
    axes[0, 0].axvline(
        best_chunk["chunk_size"], color="red", linestyle="--", alpha=0.7, label="Best"
    )
    axes[0, 0].legend()

    # Number of chunks
    axes[0, 1].bar(
        chunk_df["chunk_size"], chunk_df["num_chunks"], color="orange", alpha=0.7
    )
    ...
    # Processing time
    axes[1, 0].bar(
        chunk_df["chunk_size"], chunk_df["time_seconds"], color="red", alpha=0.7
    )
...
def plot_k_results(k_df, best_k):
    ...
    axes[1].axis("off")
    table_data = [
        [f"{row['k']}", f"{row['mrr']:.3f}", f"{row['time_seconds']:.1f}s"]
...
def print_retrieval_results(retrieval_df):
    ...
    print(f"\nBy Category:")
    category_stats = (
        retrieval_df.groupby("category")
        .agg({"mrr": "mean", "ndcg": "mean", "coverage": "mean"})
        .round(4)
    )
    print(category_stats)

    print(f"\nWorst 5 Performing Questions (by MRR):")
    worst = retrieval_df.nsmallest(5, "mrr")[
        ["question", "category", "mrr", "coverage"]
    ]
...
def print_answer_results(answer_df):
    ...
    for i, row in answer_df.head(3).iterrows():
        ...
        print(
            f"Scores: Accuracy={row['accuracy']:.1f}, Completeness={row['completeness']:.1f}, Relevance={row['relevance']:.1f}"
        )
...
def save_evaluation_results(retrieval_df, retrieval_results, answer_results):
    ...
    if "answer_quality" in evaluation_results:
        print(
            f"  Answer Accuracy: {evaluation_results['answer_quality']['avg_accuracy']:.2f}/5.00"
        )
    print("=" * 80)
...
def compare_rag_configurations(answer_sample_size=10):
    ...
    for i, (config_name, use_qe, use_hs) in enumerate(configs):
        ...
        # Run evaluation
        retrieval_results, answer_results = run_evaluation(
            answer_sample_size, config_name
        )
    ...
    for r in all_results:
        print(
            f"{r['config']:<25} {r['mrr']:<8.4f} {r['ndcg']:<8.4f} {r['coverage']:<8.1f} "
    ...
    if best_accuracy.get("accuracy"):
        print(
            f"Best Accuracy: {best_accuracy['config']} ({best_accuracy['accuracy']:.2f}/5.0)"
        )
...
Examples:
  ...
  python evaluate.py --eval --answer-sample-size 15
    ...
    # Mode selection
    parser.add_argument(
        "--all", action="store_true", help="Run both tuning and evaluation"
    )
    parser.add_argument(
        "--tune", action="store_true", help="Run hyperparameter tuning only"
    )
