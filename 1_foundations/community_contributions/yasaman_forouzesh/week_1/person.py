...
class Person:
    ...
    def __init__(self):
        ...
        self.name = "Yasaman"
        self.tools = [
            {"type": "function", "function": app_tools.record_unkown_question_json},
            {"type": "function", "function": app_tools.store_email_json},
        ]
        self.resume = ""
    ...
    def evaluate_system_prompt(self):
        system_prompt = (
            f"You are an evaluator that decids weather a email response to the user who had chat with {self.name}"
    ...
    def chat(self, message, history, session_id):
        messages = (
            [{"role": "system", "content": self.system_chat_promt()}]
            + history
        ...
        while not done:
            response = self.openai.chat.completions.create(
                model="gpt-5-mini", messages=messages, tools=self.tools
            )
    ...
    def evaluate(self, reply, history) -> validation:
        messages = [
            {"role": "user", "content": self.evaluator_user_prompt(reply, history)},
            {"role": "system", "content": self.evaluate_system_prompt()},
        ]
        resposne = self.gemeni.beta.chat.completions.parse(
            model="gemini-2.0-flash", messages=messages, response_format=validation
        )
        return resposne.choices[0].message.parsed

    def rerun(self, reply, history, feedback) -> emailResp:
        update_system_prompt = (
            self.email_system_prompt()
        ...
        messages = (
            [
                {
                    "role": "user",
                    "content": "Please provide good quality of email resposne.",
                }
            ]
            + history
        ...
        response = self.openai.beta.chat.completions.parse(
            model="gpt-5-mini", messages=messages, response_format=emailResp
        )
    ...
    def email(self, sessiondata):
        ...
        reply = self.openai.beta.chat.completions.parse(
            model="gpt-5-mini", messages=messages, response_format=emailResp
        )
        resp = reply.choices[0].message.parsed
        evaluation = self.evaluate(
            reply=reply.choices[0].message.content, history=sessiondata["history"]
        )
        ...
        try:
            with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
                server.set_debuglevel(
                    1
                )  # prints SMTP conversation to stdout for debugging
