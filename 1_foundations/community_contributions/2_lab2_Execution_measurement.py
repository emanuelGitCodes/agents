...
models_dict = {
    "openai": {"model": "gpt-5-mini", "api_key": openai_api_key, "base_url": None},
    "gemini": {
...
def get_single_model_answer(
    provider: str, details: Dict, question_messages: List[Dict]
    ...
    try:
        answer = llm_caller_with_model(
            question_messages, details["model"], details["api_key"], details["base_url"]
        )
...
def get_models_answers_parallel(question_messages, max_workers: int = 4):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        future_to_provider = {
            executor.submit(
                get_single_model_answer, provider, details, question_messages
            ): provider
            for provider, details in models_dict.items()
        }
    ...
    print(
        f"Parallel execution completed. {len(competitors)} models responded successfully."
    )


async def get_single_model_answer_async(
    provider: str, details: Dict, question_messages: List[Dict]
) -> Tuple[str, Optional[str]]:
    """
...
async def get_models_answers_async(question_messages):
    ...
    print(
        f"Async execution completed. {len(competitors)} models responded successfully."
    )
...
def compare_execution_methods(question_messages, runs_per_method=1):
    ...
    for method, data in results.items():
        print(
            f"{method.upper():>12}: {data['avg_time']:>6.2f}s avg, {data['successful_models']} models"
        )
