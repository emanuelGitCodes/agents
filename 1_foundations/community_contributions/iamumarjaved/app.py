...
class DigitalTwin:
    ...
    def __init__(self):
        ...
        self.notifier = PushoverNotifier(
            self.config["pushover_user"], self.config["pushover_token"]
        )
        ...
        if not self.documents:
            raise ValueError(
                "No documents loaded! Please add content to the me/ directory."
            )
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            ...
                            "notes": {
                                "type": "string",
                                "description": "A brief 1-line summary of what the user was asking about or interested in",
                            },
                        },
    ...
    def record_user_details(self, email: str, name: str, notes: str) -> Dict:
        ...
        return {
            "recorded": "ok",
            "message": f"Perfect! Thanks {name}. I'll be in touch soon.",
        }
    ...
    def search_knowledge_base(self, query: str, focus_area: str = None) -> Dict:
        ...
        context = self.rag_system.retriever.retrieve(
            ...
            query_expander=(
                self.rag_system.query_expander
        ...
        return {
            "success": True,
            "results": results,
            "message": f"Found {len(results)} relevant pieces",
        }
    ...
    def chat(self, message: str, history: List) -> str:
        ...
        for h in history:
            if isinstance(h, (list, tuple)) and len(h) == 2:
                ...
                converted_history.append(
                    {k: v for k, v in h.items() if k in ["role", "content"]}
                )
        ...
        if use_rag:
            query_check = self.openai.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "user",
                        "content": f"Is this query asking for specific information about someone's background, experience, or skills? Answer only 'yes' or 'no'.\n\nQuery: {message}",
                    }
                ],
                temperature=0,
            )
            should_retrieve = (
                query_check.choices[0].message.content.strip().lower() == "yes"
            )
        ...
        while not done and iteration < max_iterations:
            ...
            response = self.openai.chat.completions.create(
                model="gpt-5-mini", messages=messages, tools=self.tools, temperature=0.7
            )
...
with gr.Blocks(
    ...
    css="#chatbot {height: 600px;} .contain {max-width: 900px; margin: auto;}",
) as demo:
    gr.Markdown(
        f"""# Chat with {twin.name}
...
Features: Advanced RAG - Context-aware - Smart contact collection - Real-time notifications"""
    ...
    chatbot = gr.ChatInterface(
        ...
        textbox=gr.Textbox(
            placeholder=f"Ask me about {twin.name}'s experience, skills, or background...",
            container=False,
            scale=7,
        ),
        title=None,
