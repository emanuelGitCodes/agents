...
class HybridRetriever:
    ...
    def index_documents(
        ...
        if not all_chunks:
            raise ValueError(
                "No text chunks created from documents. Please check your document content."
            )
        ...
        for i in range(0, len(all_chunks), batch_size):
            ...
            self.collection.add(
                ...
                metadatas=[
                    {"source": doc["source"], "chunk_idx": doc["chunk_idx"]}
                    for doc in batch
                ],
            )
    ...
    def retrieve_hybrid(
        ...
        def normalize_scores(results):
            ...
            return {
                doc["id"]: (score - min_score) / range_score for doc, score in results
            }
    ...
    def rerank(
        self, query: str, documents: List[Tuple[Dict, float]], top_k: int = 5
    ) -> List[Tuple[Dict, float]]:
        if not documents:
            return []
        ...
        reranked = [
            (doc, float(score)) for (doc, _), score in zip(documents, rerank_scores)
        ]
        ...
        for q in queries:
            if method == "bm25":
                ...
                results = self.retrieve_hybrid(
                    q,
                    top_k * 2,
                    kwargs.get("bm25_weight", 0.5),
                    kwargs.get("semantic_weight", 0.5),
                )
            else:
...
class RAGSystem:
    ...
    def load_knowledge_base(
        self, documents: Dict[str, str], chunk_size: int = 500, overlap: int = 50
    ):
    ...
    def generate_answer(
        ...
        context_str = "\n\n".join(
            [
                f"[Source: {doc['source']}, Chunk {doc['chunk_idx']}]\n{doc['text']}"
                for doc in context
            ]
        )
...
Please answer the query based on the context provided above."""
        ...
        response = self.client.chat.completions.create(
            model="gpt-5-mini", messages=messages, temperature=0.7
        )
    ...
    def query(
        ...
        **kwargs,
        ...
        context = self.retriever.retrieve(
            ...
            query_expander=self.query_expander if expand_query else None,
            **kwargs,
        )
