...
class RAGEvaluator:
    ...
    def dcg_at_k(self, relevances: List[float], k: int = None) -> float:
        ...
        return relevances[0] + sum(
            rel / np.log2(i + 1) for i, rel in enumerate(relevances[1:], 2)
        )

    def ndcg_at_k(
        self, retrieved_docs: List[str], relevance_scores: Dict[str, float], k: int = 5
    ) -> float:
        retrieved_relevances = [
            relevance_scores.get(doc_id, 0.0) for doc_id in retrieved_docs[:k]
        ]
    ...
    def llm_as_judge_relevance(
        self, query: str, document: str, context: str = ""
    ) -> Dict:
  ...
  "feedback": "<brief explanation>"
    ...
    def evaluate_rag_system(
        ...
        for test_case in test_cases:
            ...
            judge_result = self.llm_as_judge_answer(
                query, response["answer"], ground_truth, context_texts
            )
    ...
    def compare_rag_methods(
        ...
        for method in methods:
            ...
            method_results = self.evaluate_rag_system(
                test_cases, rag_system, system_prompt, method
            )
...
if __name__ == "__main__":
    ...
    test_cases = create_test_cases(
        [
            ...
            (
                ...
                "List of technologies and tech stack",
            ),
            (
    ...
    csv_path, summary_path = evaluator.save_evaluation_report(
        results, name="rag_comparison"
    )
