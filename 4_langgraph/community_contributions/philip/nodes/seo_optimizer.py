...
Generate the complete SEO-optimized article."""
    ...
    seo_llm = ChatOpenAI(model="gpt-5-mini", temperature=0)
...
4. SEO score and recommendations"""
    ...
    seo_response = seo_llm.invoke(
        [
            SystemMessage(
                content="You are an SEO expert. Generate comprehensive SEO metadata."
            ),
    ...
    seo_metadata = {
        ...
        "meta_description": (
            seo_response[:160] if len(seo_response) > 160 else seo_response
        ),
        "keywords": keywords,
