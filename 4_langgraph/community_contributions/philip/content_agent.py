...
class EvaluatorOutput(BaseModel):
    ...
    word_count_met: bool = Field(
        description="Whether the content meets the required word count"
    )
    content_complete: bool = Field(
        description="Whether the content is complete (no incomplete sections)"
    )
...
class ContentAgent:
    ...
    def __init__(self):
        ...
        self.evaluator_llm = ChatOpenAI(model="gpt-5-mini")
    ...
    def evaluator(self, state: ContentState) -> ContentState:
        ...
        success_criteria = state.get(
            "success_criteria", "Content should be high quality and meet requirements"
        )
...
Note: If refinement attempts ({refinement_count}) are at maximum ({MAX_REFINEMENTS}), you may be more lenient, but still be accurate."""
    ...
    def route_after_evaluation(self, state: ContentState) -> str:
        ...
        has_issues = bool(
            evaluator_feedback.get("specific_issues")
            or evaluator_feedback.get("missing_topics")
        )
    ...
    async def run(
        self,
        message: str,
        success_criteria: str = "Content should be high quality and meet all requirements",
    ) -> Dict[str, Any]:
