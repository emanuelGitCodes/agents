...
class UrlCheckItem(BaseModel):
    ...
    reason: str = Field(
        description="Your reasoning for why this check is important for determining the URL's safety."
    )
    check: str = Field(
        description="The specific aspect to analyze (e.g., SSL validity, phishing risk, redirect behavior)."
    )
...
class UrlCheckPlan(BaseModel):
    ...
    checks: list[UrlCheckItem] = Field(
        description="A list of security aspects to analyze for this URL."
    )
