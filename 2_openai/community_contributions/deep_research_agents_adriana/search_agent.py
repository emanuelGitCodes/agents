...
class SearchOutput(BaseModel):
    summary: str = Field(
        description="Return a summary no longer than 500 words and 3-5 paragraphs."
    )
    sources: list[SourceItem] = Field(
        description="Provide a list with the sources including title and URL for each",
        min_items=3,
        max_items=5,
    )
...
search_tool = search_agent.as_tool(
    ...
    tool_description="Search web for given term and provide sources",
)
