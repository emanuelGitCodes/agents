...
class Evaluation(BaseModel):
    ...
    feedback_report: list[FeedbackInput] = Field(
        description="Feedback texts per dimension"
    )
    revised_markdown: Optional[str] = Field(
        default=None, description="Rewritten report when necessary; null if no rewrite"
    )
    change_log: Optional[list[str]] = Field(
        default=None, description="Summarizing edits; null if ni rewrite"
    )
...
eval_tool = eval_agent.as_tool(
    tool_name="evaluation",
    tool_description="Eavaluates final report (0-5); lists issues and feedback",
)
