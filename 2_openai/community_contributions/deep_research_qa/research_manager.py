...
class ResearchManager:
    ...
    async def run_with_clarification(self, query: str):
        ...
        with trace("Clarification trace", trace_id=trace_id):
            ...
            return {"questions": clarification_data.questions, "trace_id": trace_id}
    ...
    async def run_research_with_answers(self, query: str, answers: list[str]):
        ...
        with trace("Research with clarification trace", trace_id=trace_id):
            ...
            return {"report": result.final_output, "trace_id": trace_id}
...
async def evaluate_report(query: str, report: str) -> Dict[str, Any]:
    ...
    print(
        f"Evaluation complete - Score: {evaluation.overall_score}/10, Needs refinement: {evaluation.needs_refinement}"
    )
    return {
...
async def optimize_report(
    query: str, original_report: str, evaluation_feedback: str
) -> str:
...
async def _send_report_email_to_address(
    report: str, recipient_email: str
) -> Dict[str, str]:
    ...
    try:
        ...
        html_report = re.sub(
            r"\[([^\]]+)\]\(([^)]+)\)",
            r'<a href="\2" style="color: #2563eb; text-decoration: none; border-bottom: 1px solid #2563eb;" target="_blank">\1</a>',
            html_report,
        )
        ...
        html_report = re.sub(
            r"^### (.*$)",
            r'<h3 style="color: #2563eb; margin-top: 25px; margin-bottom: 10px;">\1</h3>',
            html_report,
            flags=re.MULTILINE,
        )
        html_report = re.sub(
            r"^## (.*$)",
            r'<h2 style="color: #1d4ed8; margin-top: 30px; margin-bottom: 15px;">\1</h2>',
            html_report,
            flags=re.MULTILINE,
        )
        html_report = re.sub(
            r"^# (.*$)",
            r'<h1 style="color: #1e40af; margin-top: 35px; margin-bottom: 20px;">\1</h1>',
            html_report,
            flags=re.MULTILINE,
        )
        ...
        html_report = re.sub(
            r"\*\*(.*?)\*\*",
            r'<strong style="color: #374151;">\1</strong>',
            html_report,
        )
        ...
        html_report = re.sub(
            r"^(\d+\.\s)(.*$)",
            r'<li style="margin-bottom: 8px; list-style-type: decimal;">\2</li>',
            html_report,
            flags=re.MULTILINE,
        )
        ...
        html_report = re.sub(
            r'(<li style="margin-bottom: 8px; list-style-type: decimal;">.*?</li>)',
            r'<ol style="margin: 15px 0; padding-left: 25px;">\1</ol>',
            html_report,
            flags=re.DOTALL,
        )
        html_report = re.sub(
            r'(<li style="margin-bottom: 8px;">.*?</li>)',
            r'<ul style="margin: 15px 0; padding-left: 25px;">\1</ul>',
            html_report,
            flags=re.DOTALL,
        )
        ...
        html_report = (
            '<p style="margin-bottom: 15px; line-height: 1.6;">' + html_report + "</p>"
        )
        ...
        if response.status_code == 202:
            ...
            return {
                "status": f"Email sending failed with status {response.status_code}"
            }
...
@function_tool
async def send_report_email_to_address(
    report: str, recipient_email: str
) -> Dict[str, str]:
    """Send the final research report via email to a specific address"""
...
async def run_research_with_progress(
    query: str, email_address: str = None, send_email: bool = False
...
Please improve the report based on this feedback."""
            ...
            if send_email and email_address:
                ...
                try:
                    ...
                    result = await _send_report_email_to_address(
                        final_report, email_address
                    )
