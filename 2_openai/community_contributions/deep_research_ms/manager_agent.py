...
writer = writer_agent.as_tool(
    tool_name="writer", tool_description="Generate a report from the search results"
)

email = email_agent.as_tool(
    tool_name="email", tool_description="Send an email with the report"
)
...
manager_agent = Agent(
    name="ManagerAgent", instructions=INSTRUCTIONS, model="gpt-5-mini", tools=tools
)
