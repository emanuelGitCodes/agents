    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data and data.get("success"):
                ...
                return {
                    "status": "unverifiable",
                    "reason": "unexpected result from email verification server",
                }
        else:
            return {
                "status": "unverifiable",
                "reason": f"received http response code: {response.status_code} from email verification server",
            }

    except Exception as e:
        return {
            "status": "unverifiable",
            "reason": "unexpected exception from email verification server",
        }
...
async def guardrail_against_email(ctx, agent, email):
    ...
    return GuardrailFunctionOutput(
        output_info=result, tripwire_triggered=(result["status"] != "deliverable")
    )
