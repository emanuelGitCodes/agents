...
class ClinicalQuery(BaseModel):
    ...
    disease: str = Field(
        description="The disease or cancer type (e.g., 'Non-Small Cell Lung Cancer', 'NSCLC')"
    )
    genes: list[str] = Field(
        description="List of genes mentioned (e.g., ['EGFR', 'ALK'])"
    )
    mutations: list[str] = Field(
        description="Specific mutations mentioned (e.g., ['T790M', 'L858R'])"
    )
    clinical_context: str = Field(
        description="Additional clinical context such as treatment history, patient characteristics, or specific questions"
    )
    search_focus: str = Field(
        description="The primary research focus: therapies, resistance mechanisms, clinical trials, prognosis, etc."
    )
