...
class DevHandoff(BaseModel):
    user_stories: list[str] = Field(
        description="List of user stories (as 'As a..., I want..., so that...')."
    )
    acceptance_criteria: list[str] = Field(
        description="Acceptance criteria for validating feature completion."
    )
    dev_notes: str = Field(
        description="Any technical notes, assumptions, or constraints for engineers."
    )
