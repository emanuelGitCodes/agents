...
class EmergencySpecialistHandoff(BaseModel):
    ...
    is_emergency: bool = Field(
        description="Whether this case is a medical emergency requiring immediate attention"
    )
    emergency_indicators: list[str] = Field(
        description="List of emergency warning signs or symptoms identified"
    )
    clinical_reasoning: str = Field(
        description="Your reasoning for the emergency determination"
    )
    recommended_action: str = Field(
        description="Recommended course of action (e.g., call 911, visit ER immediately, urgent care, routine appointment)"
    )
    specialist_report: str = Field(
        description="Comprehensive report of your emergency assessment"
    )
