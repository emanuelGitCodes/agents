...
class MedicineSpecialistHandoff(BaseModel):
    ...
    requires_medical_treatment: bool = Field(
        description="Whether this case requires medical treatment"
    )
    potential_diagnoses: list[str] = Field(
        description="List of potential diagnoses or conditions identified"
    )
    clinical_reasoning: str = Field(
        description="Your reasoning for the diagnosis and treatment recommendations"
    )
    recommended_medications: list[str] = Field(
        description="Recommended medications or treatments with dosages (if applicable)"
    )
    treatment_duration: str = Field(
        description="Expected duration of treatment and follow-up schedule"
    )
    specialist_report: str = Field(
        description="Comprehensive report of your medical assessment"
    )
