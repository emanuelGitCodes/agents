...
class SurgerySpecialistHandoff(BaseModel):
    ...
    requires_surgery: str = Field(
        description="Whether surgery is required: 'yes', 'no', or 'maybe' (needs further evaluation)"
    )
    surgical_procedure: str = Field(
        description="Type of surgical procedure recommended (if applicable)"
    )
    clinical_reasoning: str = Field(
        description="Your reasoning for surgical vs non-surgical management"
    )
    urgency_level: str = Field(
        description="Urgency: emergency, urgent, elective, or observation"
    )
    risks_and_benefits: str = Field(
        description="Summary of potential risks and benefits of surgery"
    )
    pre_post_operative_notes: str = Field(
        description="Pre-operative requirements and post-operative care expectations"
    )
    specialist_report: str = Field(
        description="Comprehensive report of your surgical assessment"
    )
