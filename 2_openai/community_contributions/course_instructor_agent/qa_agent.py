...
class QualityAssurance(BaseModel):
    is_satisfied: bool = Field(
        description="Whether the course is satisfied with the quality"
    )
    reason: str = Field(
        description="The reason if the satisfied or not, if not it should mention the step that needs to be improved"
    )
